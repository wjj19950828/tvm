...
def measure(tmp_file, network_args, target, n_line_per_task=None, relay_file=None):
    ...
    #data = np.load("/paddle/pr_for_tvm/tvm/models/ppocr_rec_mobile_v2/input_0923.npy")
    data = np.random.rand(1, 3, 227, 227).astype("float32")
...
def default_search(global_search_space, network_args, target, tmp_file='tmp_log.json', relay_file=None):
    if os.path.exists(tmp_file):
        os.system("rm -rf %s" % tmp_file)
    inputs, results = random_choose(global_search_space, 1)
    save_records(tmp_file, inputs, results)
    all_best_cost = measure(tmp_file, network_args, target, relay_file=relay_file)
    return all_best_cost


def random_search(global_search_space, network_args, target, total_cts=30, top_k=3, tmp_file='tmp_log.json', relay_file=None):
    ct = 0
    ...
    inputs, results = random_choose(global_search_space, 1)
    save_records(tmp_file, inputs, results)
    best_cost = measure(tmp_file, network_args, target, relay_file=relay_file)
    while ct < total_cts:
        if os.path.exists(tmp_file):
            os.system("rm -rf %s" % tmp_file)
        inputs, results = random_choose(global_search_space, top_k)
        save_records(tmp_file, inputs, results)
        cost = measure(tmp_file, network_args, target, relay_file=relay_file)
        if cost < best_cost:
            best_cost = cost
        ct += 1
