...
def get_tuning_option(tuning_args, target):
    ...
    if "cpu" in target.keys:
        ...
        tuning_opt = auto_scheduler.TuningOptions(
            ...
            num_measures_per_round=tuning_args['num_measures_per_round'],
        )
        # tuning_opt = auto_scheduler.TuningOptions(
...
def tune_and_evaluate(network_args, tuning_args, target, target_host, result_file, transfer_tune, search_type, relay_file):
    import pickle
    import time
    # with open('relay_files/DBFace/relay.pkl', 'rb') as f:
    #     mod, params = pickle.load(f)
    # print("test_wjj:",relay_file)
    ...
    print("search cost time:", time.time() - start_time)
    best_by_targetkey, _ = local_search(log_file)
    if search_type == "random":
        prof_res = random_search(best_by_targetkey, network_args, target, relay_file=relay_file)
    else:
        prof_res = default_search(best_by_targetkey, network_args, target, relay_file=relay_file)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--relay-file", type=str, help="Write relay file path")
    ...
    tuning_args = {
        ...
        "num_measures_per_round": 30,
    }

    tune_and_evaluate(network_args, tuning_args, target, args.target_host,
                      args.result_file, args.transfer_tune, args.search_type, args.relay_file)
