...
class Dataset:
    ...
    def update_from_measure_pairs(self, inputs: List[MeasureInput], results: List[MeasureResult]):
        ...
        for task, (inputs, results) in new_data.items():
            ...
            self.load_task_data(task, features, normalized_throughputs)
    ...
    def load_task_data(self, task: LearningTask, features, throughputs, min_latency=None):
        ...
        if task not in self.features:
            ...
            try:
                ...
                self.throughputs[task] = np.concatenate([self.throughputs[task], throughputs])
            except ValueError:
                ...
                new_throughputs = list(self.throughputs[task])
                new_throughputs.extend(throughputs)
                self.throughputs[task] = np.array(new_throughputs, dtype=object)
