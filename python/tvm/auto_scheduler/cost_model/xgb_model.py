...
class XGBModelInternal:
    ...
    def _fit_a_model(self, train_set, valid_set=None, valid_train_set=None):
        ...
        print("test_wjj_model_path:",self.model_path)
        self.load(self.model_path)
        # test
        self.xgb_params.update({'process_type': 'update',
                                'updater' : 'refresh',
                                'refresh_leaf' : True})
        # Train a new model
        bst = xgb.train(
...
class XGBModel(PythonBasedModel):
    ...
    def __init__(self, few_shot_learning="base_only", verbose_eval=25,
                 num_warmup_sample=100, seed=None, disable_update=False, model_path=None):
        super().__init__()
